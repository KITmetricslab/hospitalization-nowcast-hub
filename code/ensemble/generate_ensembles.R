...
for(te in teams){
  ...
  if(file_forecast_date %in% fls){
    ...
    locations_to_remove <- names(tab_locations)[tab_locations < (27*8)]
    ...
    check5 <- all(paste(-2:-28, "day ahead inc hosp") %in% dat_temp$target)
    if(!check5) cat("   Not all horizons between -2 and -28 days addressed in", te, "\n")
...
write.csv(mean_ensemble, file = paste0("data-processed/NowcastHub-MeanEnsemble/",
                                       forecast_date, "-NowcastHub-MeanEnsemble.csv"))
...
write.csv(median_ensemble, file = paste0("data-processed/NowcastHub-MedianEnsemble/",
                                       forecast_date, "-NowcastHub-MedianEnsemble.csv"))